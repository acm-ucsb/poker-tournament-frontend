...
export function TournamentStats() {
  ...
  const [remainingPlayers, setRemainingPlayers] = useState<number>();

  useEffect(() => {
    async function fetchRemainingPlayers() {
      const { data, error } = await supabase.rpc("count_remaining_teams");
      if (error) {
...
export function LeaderboardTracker() {
  ...
  return (
    <div className={cardBase}>
      ...
      <div className="mt-3">
        <ol
          className={`space-y-2 pr-2 ${shouldScroll ? "h-[17rem] overflow-y-auto" : ""}`}
        >
          {players.map((p, i) => (
            <li
              key={p.id}
              className="flex items-center gap-3 p-2 rounded-md hover:bg-[rgba(96,239,134,0.04)] h-12"
            >
              <div
