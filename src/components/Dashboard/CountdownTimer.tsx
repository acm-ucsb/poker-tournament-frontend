...
export function CountdownTimer({ targetDate }: { targetDate: Date }) {
  ...
  const has_ended = () => (timeLeft.days == 0) && (timeLeft.hours == 0) && (timeLeft.minutes == 0) && (timeLeft.seconds == 0);

  const pad = (num: number) => String(num).padStart(2, "0");

  return (
    <div className={`flex flex-col items-center justify-center space-y-4 select-none pb-4 ${has_ended() ? "opacity-30" : ""}`}>
